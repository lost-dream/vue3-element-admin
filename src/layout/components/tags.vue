<template>
  <div class="position">
    ...
    >
      ...
      <li class="px-4 py-2 cursor-pointer hover:bg-gray-200" @click="closeAll">
        关闭所有
      </li>
    </ul>
  </div>
...
const rightMenu = (
  ...
  // 当前右键的那个标签
  let currentRightTags: any
  const contextRightMenu = (v: any, event: MouseEvent) => {
    currentRightTags = v
...
// 标签页滚动
const tagScroll = (store: Store<AllState>) => {
...
export default defineComponent({
  ...
  setup() {
    ...
    const removeTagNav = (v: any) =>
      store.commit('layout/removeTagNav', { cPath: route.path, tagsList: v })
